...
namespace pointers
{
	void scan_all()
	{
		...
		g_gxtLabels = scan("GL", "48 8D 0D ? ? ? ? E8 ? ? ? ? 8B 0D ? ? ? ? 48 8B 5C 24 30").mov().as<decltype(
			g_gxtLabels)>();
		g_getGxtLabelFromTable = scan("GGLFT", "E8 ? ? ? ? 48 8D 4B 0B").call().as<decltype(g_getGxtLabelFromTable)>();
		g_getJoaatedGxtLabelFromTable = scan("GJGLFT", "48 83 EC 28 E8 ? ? ? ? 48 85 C0 75 34").as<decltype(
			g_getJoaatedGxtLabelFromTable)>();
		g_cTaskJumpConstructor = scan("CTJC", "48 89 5C 24 08 89 54 24 10 57 48 83 EC 30 0F").as<decltype(
			g_cTaskJumpConstructor)>();
		g_cTaskFallConstructor = scan("CTFC", "48 89 5C 24 08 89 54 24 10 57 48 83 EC 20 48 8B D9 E8").as<decltype(
			g_cTaskFallConstructor)>();
		g_scGetGameInfoIndex = scan("SGGII", "E8 ? ? ? ? 85 C0 78 27 3B 47 14 7D 1E 48 98 48 8B D6 48 69 C8",
		                            {"socialclub.dll"}).call().as<decltype(g_scGetGameInfoIndex)>();
		g_joinBySessionInfo = scan("JBSI", "89 6C 24 ? 4C 89 74 24 ? E8 ? ? ? ? EB").mov().as<decltype(
			g_joinBySessionInfo)>();
		g_textureStore = scan("TS", "48 8D 0D ? ? ? ? E8 ? ? ? ? 8B 45 EC").mov().as<decltype(g_textureStore)>();
		g_scGameInfo = scan("SGI", "48 8D 05 ? ? ? ? 48 03 F8 44 8B 47 14 48 8D 57 20 E8 ? ? ? ? 85",
		                    {"socialclub.dll"}).mov().as<decltype(g_scGameInfo)>();
		...
		g_networkPlayerMgr = scan("NPM", "48 8B 0D ? ? ? ? 8A D3 48 8B 01 FF 50 ? 4C 8B 07").mov().as<decltype(
			g_networkPlayerMgr)>();
		g_networkObjectMgr = scan("NOM", "48 8B 0D ? ? ? ? 45 33 C0 E8 ? ? ? ? 33 FF 4C 8B F0").mov().as<decltype(
			g_networkObjectMgr)>();
		...
		g_httpStartRequest = scan("HSR", "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 20 48 8B D9 48 81 C1 28 05").as<
			decltype(g_httpStartRequest)>();
		g_scriptHandlerMgr = scan("SHM", "74 17 48 8B C8 E8 ? ? ? ? 48 8D 0D").add(0xA).mov().as<decltype(
			g_scriptHandlerMgr)>();
		...
		g_nativeRegistrationTable = scan("NRT", "48 8D 0D ? ? ? ? 48 8B 14 FA").mov().as<decltype(
			g_nativeRegistrationTable)>();
		...
		g_threadId = scan("TI", "8B 15 ? ? ? ? 48 8B 05 ? ? ? ? FF C2 89 15 ? ? ? ? 48 8B 0C F8").lea().as<decltype(
			g_threadId)>();
