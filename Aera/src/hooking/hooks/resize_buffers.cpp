                              ...
                              DXGI_FORMAT new_format, UINT swap_chain_flags)
{
	if (g_running)
	{
		...
		const auto result = g_hooking->m_DX.get_original<decltype(&resize_buffers)>(g_swapchain_resizebuffers_index)(
	...
	return g_hooking->m_DX.get_original<decltype(&resize_buffers)>(g_swapchain_resizebuffers_index)(
		swap_chain, buffer_count, width, height, new_format, swap_chain_flags);
}
