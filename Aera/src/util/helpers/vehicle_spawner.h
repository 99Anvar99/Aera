...
namespace util::vehicle
{
    ...
    constexpr uint32_t invalid_vehicles[] = { "arbitergt"_joaat, "astron2"_joaat, "cyclone2"_joaat, "ignus2"_joaat, "s95"_joaat };
    ...
    inline void spawn_vehicle(const Hash hash)
    {
	    ...
	    if (Vehicle previous_vehicle; seamless_re_enter_vehicle && (previous_vehicle = PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), TRUE)) != NULL)
	    {
	    ...
	    if (set_vehicle_maxed_when_spawned)
	    {
		    ...
		    for (int j = 0; j < 50; ++j)
		    {
			    VEHICLE::SET_VEHICLE_MOD(new_spawned_vehicle, j, VEHICLE::GET_NUM_VEHICLE_MODS(new_spawned_vehicle, j) - 1, TRUE);
		    }
